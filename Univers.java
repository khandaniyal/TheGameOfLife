...
public class Univers extends JLabel{
    ...
    public int countNeighbours(int x, int y, Univers[][] univers){
        ...
        for(int i = x - 1; i <= x + 1; i++) {
            for(int j = y - 1; j <= y + 1; j++) {
                try {
                    if(univers[i][j].isUniverse()) neighbours++; //counts each cell
                }catch(Exception e){ }
            }
        }
        if(univers[x][y].isUniverse()) neighbours--; //reduces the cell count until you get the count of the surrounding univers of the alive cell
        return neighbours;
    }
}
